...
import { ClockCircleFilled, CloseCircleFilled, MinusCircleFilled, PlusCircleFilled } from '@ant-design/icons';
    ...
    try {
      ...
      querySnapshot1.forEach((element,index)=>{
          ...
          for(let i=0;i<temptras.allobjects.length;i++){
            if(Number(temptras.allobjects[i].feeamount)-Number(values.feeamount)>=0){
                temptras.allobjects[i].feeamount=Number(temptras.allobjects[i].feeamount)-Number(values.feeamount)
            }
            else{
                temptras.allobjects[i].feeamount=0
            }
          }
